    ...
    public partial class dashboard : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    Response.Redirect("~/_content_pages/dashboard-new");
            ...
            if (WebConfigurationManager.AppSettings["Dbsync"] == "Yes")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                   ...
                   // LblDestinationHeading.Text = WebConfigurationManager.AppSettings["DestinationSite"];
                    ...
                    //
                    lblInstruction.Text = getdt.GetMOMandInstructionCount("2").ToString();
                    lblMOM.Text = getdt.GetMOMandInstructionCount("1").ToString();
                    lblArchived.Text = getdt.GetDashboardArchivedDocs(new Guid(DDlProject.SelectedValue)).ToString();
                    //
...
fontSize: 12,
            ...
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        ...
                        strScript.AppendLine("['" + documentName + "', " + count + ", " + delay + ", 'blue', 'red', '', ''],");
                    }
                    ...
                    strScript.AppendLine("var options = {");
                    strScript.AppendLine("    is3D: true,");
                    strScript.AppendLine("    legend: { position: 'none' },");
                    strScript.AppendLine("    fontSize: 11,");
                    strScript.AppendLine("    isStacked: true,");
                    strScript.AppendLine("    height: 300,");
                    strScript.AppendLine("    chartArea: {");
                    strScript.AppendLine("        left: '25%',");
                    strScript.AppendLine("        top: '5%',");
                    strScript.AppendLine("        height: '100%',");
                    strScript.AppendLine("        width: '61%'");
                    strScript.AppendLine("    },");
                    strScript.AppendLine("    bars: 'horizontal',");
                    strScript.AppendLine("    annotations: {");
                    strScript.AppendLine("        alwaysOutside: true,");
                    strScript.AppendLine("        textStyle: {");
                    strScript.AppendLine("            fontSize: 12,");
                    strScript.AppendLine("            color: 'red'");
                    strScript.AppendLine("        }");
                    strScript.AppendLine("    },");
                    strScript.AppendLine("    axes: {");
                    strScript.AppendLine("        x: {");
                    strScript.AppendLine("            0: { side: 'top', label: 'Percentage' }");
                    strScript.AppendLine("        }");
                    strScript.AppendLine("    },");
                    strScript.AppendLine("    hAxis: {");
                    strScript.AppendLine("        minValue: 0");
                    strScript.AppendLine("    }");
                    strScript.AppendLine("};");
                ...
                if (dataSet.Tables[0].Rows.Count > 0)
                {
                    ...
                    lblTotalMasterListDocuments.Text = $"<span style='color: darkgreen;'>Total Master List Documents : {totalDocuments}</span> &nbsp;&nbsp; " +
        $"<span style='color: blue;'>Pending : {pendingCount}</span> &nbsp;&nbsp; " +
        $"<span style='color: red;'>Delayed : {delayedCount}</span>";
                }
        ...
        private void Bind_ResourceGraph(string WorkpackageUID)
        {
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    strScript.AppendLine("<script type=\"text/javascript\">");
                    strScript.AppendLine("google.charts.load('current', {'packages':['bar']});");
                    ...
                    strScript.AppendLine("var options = {");
                    strScript.AppendLine("chart: {");
                   // strScript.AppendLine("title: 'Resource Deployment',");
                    strScript.AppendLine("title: 'Cumulative(Planned and Deployed) Values',");
                    strScript.AppendLine("},");
                    strScript.AppendLine("colors: ['#1b9e77', '#7570b3'],");
                    strScript.AppendLine("bars: 'horizontal',");
                    strScript.AppendLine("legend: { position: 'none' }");
                    strScript.AppendLine("};");
        ...
        //            strScript.AppendLine("    var selection = chart.getSelection();");
        //            strScript.AppendLine("    if (selection.length > 0) {");
        //            strScript.AppendLine("        var colLabel = data.getColumnLabel(selection[0].column);");
        //            strScript.AppendLine("        var mydata = data.getValue(selection[0].row, 0);");
        //            strScript.AppendLine("    }");
        //            strScript.AppendLine("}");
