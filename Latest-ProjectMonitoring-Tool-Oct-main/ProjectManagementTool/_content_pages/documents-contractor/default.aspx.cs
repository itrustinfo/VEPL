...
namespace ProjectManagementTool._content_pages.documents_contractor
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    //
                    if ((Session["EnggType"].ToString() == "EE" || Session["EnggType"].ToString() == "PM" || Session["EnggType"].ToString() == "TL") && Request.QueryString["type"].ToString() == "Ontb")
                    {
                        btnArchive.Visible = true;
                    }
                    else
                    {
                        GrdDocuments.Columns[26].Visible = false;
                        btnArchive.Visible = false;
                    }

                    //
                    if (getdt.GetUserClientType(Guid.NewGuid(), Session["UserUID"].ToString()) != "Recon")
                    {
        ...
        private void FirstData(string type)
        {
         ...
         .CopyToDataTable();
             ...
             .CopyToDataTable();
                 ...
                 else if (Request.QueryString["type"].ToString() == "Archived")
                {
                    DataSet ds = getdt.GetDashboardArchivedDocs_Details(new Guid(Request.QueryString["PrjUID"]));
                    lblTotalcount.Text = "Total Count : " + ds.Tables[0].Rows.Count;
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        GrdDocuments.DataSource = ds.Tables[0].AsEnumerable()
             ...
             .CopyToDataTable();
                    ...
                    LblDocumentHeading.Text = "Document List for Archived Docs";
                    ...
                    GrdDocuments.Columns[16].Visible = false;
                    GrdDocuments.Columns[17].Visible = false;
                    GrdDocuments.Columns[18].Visible = false;
                    GrdDocuments.Columns[19].Visible = false;
                    GrdDocuments.Columns[20].Visible = false;
                    GrdDocuments.Columns[21].Visible = false;
                    GrdDocuments.Columns[22].Visible = false;
                    GrdDocuments.Columns[23].Visible = false;
                    GrdDocuments.Columns[24].Visible = false;
        ...
        protected void GrdDocuments_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                //added on 13/10/2023
                Button btnunarchive = (Button)e.Row.Cells[27].FindControl("btnunarchive");
                if (Request.QueryString["type"].ToString() == "Archived")
                {
                    if (Session["UserUID"].ToString() == e.Row.Cells[28].Text)
                    {
                ...
                if (e.Row.Cells[6].Text == "Reconciliation")
                {
                    string dateC = e.Row.Cells[10].Text;
                    DateTime sdate = Convert.ToDateTime(dateC.Split('/')[1] + "/" + dateC.Split('/')[0] + "/"+ dateC.Split('/')[2]);

                    if(DateTime.Now > sdate.AddDays(3))
                    {
                        e.Row.BackColor = e.Row.BackColor = System.Drawing.Color.OrangeRed;
                    }
                }
                        ...
                        if (dsNxtUser.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow druser in dsNxtUser.Tables[0].Rows)
                            {
                                if (Session["UserUID"].ToString().ToUpper() == druser["Approver"].ToString().ToUpper())
                                {
                                    ...
                                    //}
                                    //else 
                                    if (ds.Tables[0].Rows[0]["ActivityType"].ToString().ToLower().Contains("back to pmc"))
                                    {
                ...
                //
                //added for submission flow
                if (e.Row.Cells[4 + 2].Text == "Sent to PMC")
                {
                    e.Row.Cells[4 + 2].Text = "Accepted" + "<br/>" + "(Under PMC Review)";

                }
                else if (e.Row.Cells[4 + 2].Text == "Accepted-PMC")
                {
                    e.Row.Cells[4 + 2].Text = "Accepted" + "<br/>" + "(PMC Review Completed)"; ;
                }

                //added for submission flow
                if (e.Row.Cells[21].Text.ToUpper() != "95AF4084-3F07-4184-8E3F-1CA3A916EE59")
                {
                    e.Row.Cells[23].Text = "";
                    e.Row.Cells[24].Text = "";
                }
        ...
        protected void GrdDocuments_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                getdt.UpdateArchivedDocs(new Guid(UID), new Guid(Session["UserUID"].ToString()), "N", "N");
                ...
                foreach (GridViewRow row in GrdDocuments.Rows)
                {
                   ...
                   string documentname = getdt.GetActualDocumentName_by_ActualDocumentUID(new Guid(DocumentUID));
                    ...
                    int Cnt = getdt.InsertorUpdateDocumentStatus(StatusUID, new Guid(DocumentUID), 1, status, 0, CDate1, DocPath,
                        new Guid(Session["UserUID"].ToString()), remarks, status, "", CDate1, CoverPagePath, "Current", "ONTB",remarks);
                        if (Cnt > 0)
                        {
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                sHtmlString += "<div style='width:100%; float:left;'><table style='width:100%;'>" +
                                               ...
                                               "<tr><td><b>Originator Ref. Number </b></td><td style='text-align:center;'><b>:</b></td><td>" + OriginatorRefNo + "</td></tr>" +
                                                    "<tr><td><b>PMC Ref. Number</b></td><td style='text-align:center;'><b>:</b></td><td>" + ProjectRefNo + "</td></tr>" +
        ...
        void BindFlowPendingDocPage(List<string> Flow)
        {
            //string str[] = Flow.Distinct().ToList();
            //DataTable ds = getdt.GetDocumentFlow().AsEnumerable().Where(r => r.Field<string>("Flow_Name").Equals("Works A") || r.Field<string>("Flow_Name").Equals("Works B") || r.Field<string>("Flow_Name").Equals("Vendor Approval")).CopyToDataTable();
            DataTable ds = getdt.GetDocumentFlow();
            ...
            if (ds != null && ds.Rows.Count > 0)
            {
                ...
                DropDownList1.DataTextField = "Flow_Name";
                DropDownList1.DataValueField = "FlowMasterUID";
        ...
        {
            // Trigger a postback for DropDownList1 to handle its SelectedIndexChanged event
     ...
     .CopyToDataTable();
            ...
            try
            {
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        BindFlowPendingDocPage(FlowUID);
                        GrdDocuments.DataSource = getdt.GetNextUserDocuments_pending(new Guid(Request.QueryString["PrjUID"]), new Guid(Request.QueryString["WkpgUID"]), str).Tables[0].AsEnumerable()
                 .OrderByDescending(r => r.Field<DateTime>("IncomingRec_Date"))
                 .CopyToDataTable();
                        GrdDocuments.DataBind();
        ...
        protected void btnArchive_Click(object sender, EventArgs e) //Archive
        {
            try
            {
                foreach (GridViewRow row in GrdDocuments.Rows)
                {
                    string DocumentUID = row.Cells[0 + 2].Text;
                    CheckBox chkSelect = (CheckBox)row.FindControl("chkArchive");
                    if(chkSelect.Checked)
                    {
                        getdt.UpdateArchivedDocs(new Guid(DocumentUID), new Guid(Session["UserUID"].ToString()),"A","Y");
        ...
        protected void DropDownList1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                // Get the selected flow from the DropDownList
                string selectedFlow = DropDownList1.SelectedValue;
                string str = HttpContext.Current.Session["docuids"].ToString();
                if (str.Contains(","))
                {
                    str = str.Substring(0, str.Length - 1);
                }
                // Get the original dataset that was used to populate the GridView initially
                DataSet originalDataSet = getdt.GetNextUserDocuments_pending(
                    new Guid(Request.QueryString["PrjUID"]),
                    new Guid(Request.QueryString["WkpgUID"]),
                    str);

                // Filter the dataset based on the selected flow
                DataTable filteredTable = originalDataSet.Tables[0].AsEnumerable()
                    .Where(row => row.Field<Guid>("FlowUID").ToString() == selectedFlow)
                    .OrderByDescending(row => row.Field<DateTime>("IncomingRec_Date"))
                    .CopyToDataTable();

                // Bind the filtered data to the GridView
                GrdDocuments.DataSource = filteredTable;
                GrdDocuments.DataBind();
                lblTotalcount.Text = "Total Count : " + filteredTable.Rows.Count;
            }
            catch (Exception ex)
            {
                // Handle exceptions
