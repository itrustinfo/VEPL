    ...
    public partial class add_documentstatus : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    //------------------------------
                    //
                    if (DDlStatus.SelectedItem.ToString().ToLower().Contains("back to pmc") || DDlStatus.SelectedItem.ToString().ToLower().Contains("sent to pmc"))
                    {
                    ...
                    if (Session["IsONTB"].ToString() == "Y")
                    {
                        if (Session["EnggType"] != null)
                        {
                            if ((Session["EnggType"].ToString() == "DTL" || Session["EnggType"].ToString() == "PM" || Session["EnggType"].ToString() == "TL") && FlowName == "Submission Flow")
                                lnkcommentsheet.Visible = true;
                        }
                    }
                    ...
                    if (getdata.GetFlowTypeBySubmittalUID(new Guid(getdata.GetSubmittalUID_By_ActualDocumentUID(new Guid(Request.QueryString["DocID"])))) == "STP-OB")
                    {
                        ...
                        else if (Session["IsONTB"].ToString() == "Y")
                        {
                            ...
                            lblrefno.InnerText = "PMC Ref. Number";
                        }
                        //
                    ...
                    //for letter wihtdrawn status hide all the upload options
                    if (DDlStatus.SelectedItem.ToString().ToLower().Contains("letter withdrawn"))
                    {
                        divReviewFile.Visible = false;
                        divFileAttachmentDoc.Visible = false;
                        divCD.Visible = false;
                        divCUpload.Visible = false;
                        txtcomments.Text = "letter withdrawn";


                    }
                    //
                    if (FlowName == "Contractor Correspondence-BWSSB" && (Session["EnggType"].ToString() == "DTL" || Session["EnggType"].ToString() == "PM" || Session["EnggType"].ToString() == "TL"))
                    {
                        divReviewFile.Visible = false;
                        divFileAttachmentDoc.Visible = false;
                        divCD.Visible = false;
        ...
        private void BindStatus()
        {
            ...
            if (ds1.Tables[0].Rows.Count > 0)
            {
                ...
                DDlStatus.DataBind();
                //added on 13/07/2022
                if (ds1.Tables[0].Rows[0]["ActivityType"].ToString().Contains("Code A"))
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        if (dr["Update_Status"].ToString().Contains("Code A"))
                        {
                            DDlStatus.SelectedValue = dr["Update_Status"].ToString();
                        }
                    }
                }
                else if (ds1.Tables[0].Rows[0]["ActivityType"].ToString().Contains("Code B"))
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        if (dr["Update_Status"].ToString().Contains("Code B"))
                        {
                            DDlStatus.SelectedValue = dr["Update_Status"].ToString();
                        }
                    }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e) //final submit
        {
            ...
            if (DDlStatus.SelectedItem.ToString() == "Closed" || FlowType  == "STP" || FlowType == "STP-C" || FlowType == "STP-OB")
            {
                try
                {
                    ...
                    if (ViewState["FlowName"].ToString() == "Works A" && DDlStatus.SelectedItem.ToString().Contains("Accepted-PMC"))
                    {
                    ...
                    else if (ViewState["FlowName"].ToString() == "Submission Flow" && DDlStatus.SelectedItem.ToString().Contains("Accepted-PMC"))
                    {
                        ...
                        {
                            DataSet dsMUSers = getdata.GetSenttoPMCUsers(new Guid(Request.QueryString["DocID"].ToString()));// getdata.GetNextUser_By_DocumentUID(new Guid(Request.QueryString["DocID"].ToString()), 3);
                            string FlowUID = getdata.GetFlowUIDBySubmittalUID(new Guid(getdata.GetSubmittalUID_By_ActualDocumentUID(new Guid(Request.QueryString["DocID"]))));
                            if (dsMUSers.Tables[0].Rows.Count > 0)
                            {
                                foreach (DataRow druser in dsMUSers.Tables[0].Rows)
                    ...
                    //
                    //added on 18/07/2023 for sending individual users instead of all users for CP24
                    if (DDlStatus.SelectedItem.ToString().ToLower().Contains("sent to pmc"))
                    {
                        divUSers.Attributes.Add("style", "display:block");
                        List<ListItem> selectedItems = chkUserList.Items.Cast<ListItem>()
                                   .Where(li => li.Selected)
                        ...
                        foreach (var eachSelectedItems in selectedItems)
                        {
                            int cnt = getdata.InsertSent_To_PMC_Users(Guid.NewGuid(), new Guid(Request.QueryString["DocID"].ToString()), new Guid(eachSelectedItems.Value));
                            Comments = Comments + "<br/> ------------------------------------------ <br/> (" + getdata.getUserNameby_UID(new Guid(eachSelectedItems.Value)) + ") Pending <br/>";
                    ...
                    else
                    {
                        ...
                        string for_commentsheet = txtcomments.Text.Replace(Environment.NewLine, "<br/><br/>");
                        if (Session["EnggType"] != null)
                        {
                            if (Session["EnggType"].ToString() == "DTL" || Session["EnggType"].ToString() == "PM" || Session["EnggType"].ToString() == "TL")
                            {
                                for_commentsheet = txtcomments.Text.Replace(Environment.NewLine, "<br/><br/>");
                            }
                        }
                        Comments = Comments.Replace(Environment.NewLine, "<br/><br/>");
                        //
                        //
                        double Version = 1;
                        ...
                        {
                            Version = Convert.ToDouble(dslatestVersion.Tables[0].Rows[0]["Doc_Version"].ToString());
                        ...
                        int Cnt = getdata.InsertorUpdateDocumentStatus(StatusUID, new Guid(Request.QueryString["DocID"].ToString()), 1, Status, 0, CDate1, DocPath,
                            new Guid(Session["UserUID"].ToString()), Comments, Status, txtrefNumber.Text, Document_Date, CoverPagePath, RBLDocumentStatusUpdate.SelectedValue, RBLOriginator.SelectedValue, for_commentsheet);
                        if (Cnt > 0)
                        {
                            ...
                            if (storemail)
                            {
                                if (ds.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        sHtmlString += "<div style='float:left; width:7%;'><img src='https://vvalley-stp.itrustinfo.com/_assets/images/viswaraj%20_logo.jpg' width='80' /></div>";

                                        RefNostring = "PMC Ref Number";
                                    }
                                    ...
                                    if (WebConfigurationManager.AppSettings["Domain"] == "NJSEI")
                                    {
                                        sHtmlString += "<div style='float:left; width:7%;'><img src='https://dm.njsei.com/_assets/images/NJSEI%20Logo.jpg' width='80' /></div>";
                                        RefNostring = "NJSEI Ref Number";
                                    }
                                    else
                                    {
                ...
                else
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //sDate2 = sDate2.Split('/')[1] + "/" + sDate2.Split('/')[0] + "/" + sDate2.Split('/')[2];
                            //CDate2 = Convert.ToDateTime(sDate2);
                            string for_commentsheet = txtcomments.Text;
                            int Cnt = getdata.InsertorUpdateDocumentStatus(StatusUID, new Guid(Request.QueryString["DocID"].ToString()), 1, DDlStatus.SelectedItem.Text, 0, CDate1, DocPath,
                                new Guid(Session["UserUID"].ToString()), txtcomments.Text, DDlStatus.SelectedItem.Text, txtrefNumber.Text, Document_Date, CoverPagePath, RBLDocumentStatusUpdate.SelectedValue, RBLOriginator.SelectedValue, for_commentsheet);
                            if (Cnt > 0)
                            {
                                //store blob for document status tables
                                //for blob
        ...
        private void BindOriginator()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                else if (Session["IsONTB"].ToString() == "Y")
                {
                    ...
                    //}
                    if (RBLOriginator.Items.FindByValue("PMC") != null)
                    {
                        RBLOriginator.Items.FindByValue("PMC").Selected = true;
                    }
                }
        ...
        private void GetAllUSersforPMC()
        {
            try
            {
                if (DDlStatus.SelectedItem.ToString().ToLower().Contains("sent to pmc"))
                {
                    DataSet dsMusers = getdata.GetAllUsersforPMC(new Guid(Request.QueryString["DocID"].ToString()), 4);
                    if (dsMusers.Tables[0].Rows.Count > 0)
                    {
        ...
        protected void lnkcommentsheet_Click(object sender, EventArgs e)
        {
            divcommentsheet.Visible = true;
            btnSubmit.Visible = false;
            divmainsheet.Visible = false;
            ...
            if (dslatestVersion.Tables[0].Rows.Count > 0)
            {
                Version = int.Parse(dslatestVersion.Tables[0].Rows[0]["Doc_Version"].ToString());

            }
            DataSet dsdata = getdata.GetCommentSheetForDTL(new Guid(Request.QueryString["DocID"]), Version);
            if (Session["EnggType"].ToString() == "TL")
            {
                dsdata = getdata.GetCommentSheetForTL(new Guid(Request.QueryString["DocID"]), Version);
            }
            else if (Session["EnggType"].ToString() == "PC")
            {
                dsdata = getdata.GetCommentSheetForPC(new Guid(Request.QueryString["DocID"]), Version);
            }

            GrdDataList.DataSource = dsdata;
            int new_serial_no = dsdata.Tables[0].Rows.Count + 1;

            dsdata.Tables[0].Rows.Add(new Object[] { new_serial_no, "", "" });
            ...
            //store the session for fileupload
        ...
        {
            divcommentsheet.Visible = false;
            btnSubmit.Visible = true;
            divmainsheet.Visible = true;
            btnSaveComments.Visible = false;
            //
            int count = GrdDataList.Rows.Count;
            int tcount = 0;
            txtcomments.Text = "";
            foreach (GridViewRow item in GrdDataList.Rows)
            {
                ...
                CheckBox chkselect = item.FindControl("chkselectcomment") as CheckBox;
                //
                //if (tcount == count)
                //{
                //    txtcomments.Text += txtBox.Text + Environment.NewLine;
                //}
                //else
                //{
                //    //txtcomments.Text += Session["Username"] + " added - " +  txtBox.Text + Environment.NewLine;
                //    txtcomments.Text += txtBox.Text + Environment.NewLine;
                //}
        ...
        protected void GrdDataList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //
                TextBox txtBox = e.Row.FindControl("TextBox1") as TextBox;
                txtBox.Text = txtBox.Text.Replace("<br/>", "");
            }
        }
    }
}