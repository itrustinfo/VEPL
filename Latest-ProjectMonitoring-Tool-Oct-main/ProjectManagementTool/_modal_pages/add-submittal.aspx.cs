...
namespace ProjectManagementTool._modal_pages
{
    public partial class add_submittal : System.Web.UI.Page
    {
        ...
        private void BindDocumentFlows()
        {
            ...
            //}
            if (Session["EnggType"].ToString() == "PC")
                 Usertype = "DTL";
            if (Session["EnggType"].ToString() == "DTL" || Session["EnggType"].ToString() == "PC" || Session["EnggType"].ToString() == "EE" || Session["EnggType"].ToString() == "ACE" || Session["EnggType"].ToString() == "CE")
            {
        ...
        private void LoadDropDowns()
        {
            try
            {
                ...
                if (FlowType == "STP" && FlowName == "Submission Flow")
                {
                    //WorkPackageCategory_UID
                    dsAllCategories = getdata.GetWorkpackageCategory(new Guid(WorkPackageUID));
                    foreach (DataRow dr in dsAllCategories.Rows)
                    {
                        dsMusers = getdata.FlowMasterUser_Select(new Guid(DDLDocumentFlow.SelectedValue), new Guid(WorkPackageUID), new Guid(dr["WorkPackageCategory_UID"].ToString()), 4, updatedUsers);
                        foreach (DataRow drM in dsMusers.Rows)
                        {
                            ddlReviewer.Items.Add(new ListItem(text: drM["Name"].ToString(), value: drM["UserUID"].ToString()));
