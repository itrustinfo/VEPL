...
namespace ProjectManagementTool._modal_pages
{
    public partial class add_workpackage : System.Web.UI.Page
    {
        ...
        private void BindWorkpackageOption()
        {
            ...
            DataSet ds = getdt.Workpackageoption_SelectBy_OptionFor("ONTB");
        ...
        private void BindUsers()
        {
            ...
            chkWrkPckgUsers.DataTextField = "UserName";
            chkWrkPckgUsers.DataValueField = "UserUID";
            chkWrkPckgUsers.DataSource = ds;
            chkWrkPckgUsers.DataBind();
        ...
        private void BindWorkPackage()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //
                if (ds.Tables[0].Rows[0]["ExtendedEndDate1"].ToString() != null && ds.Tables[0].Rows[0]["ExtendedEndDate1"].ToString() != "")
                {
                    dtExtendedEndDate1.Text = Convert.ToDateTime(ds.Tables[0].Rows[0]["ExtendedEndDate1"].ToString()).ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                }
                if (ds.Tables[0].Rows[0]["ExtendedEndDate2"].ToString() != null && ds.Tables[0].Rows[0]["ExtendedEndDate2"].ToString() != "")
                {
                    dtExtendedEndDate2.Text = Convert.ToDateTime(ds.Tables[0].Rows[0]["ExtendedEndDate2"].ToString()).ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                }
                if (ds.Tables[0].Rows[0]["ExtendedEndDate3"].ToString() != null && ds.Tables[0].Rows[0]["ExtendedEndDate3"].ToString() != "")
                {
                    dtExtendedEndDate3.Text = Convert.ToDateTime(ds.Tables[0].Rows[0]["ExtendedEndDate3"].ToString()).ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                }
                if (ds.Tables[0].Rows[0]["ExtendedEndDate4"].ToString() != null && ds.Tables[0].Rows[0]["ExtendedEndDate4"].ToString() != "")
                {
                    dtExtendedEndDate4.Text = Convert.ToDateTime(ds.Tables[0].Rows[0]["ExtendedEndDate4"].ToString()).ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                }
                // added on 25/01/2022
        ...
        private void BindAssignedUser()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    foreach (ListItem item in chkWrkPckgUsers.Items)
                    {
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //Page.Validate("add");
                    int ItemsCount = (from ListItem li in chkWrkPckgUsers.Items
                    ...
                    else
                    {
                        ...
                        string sDate1 = "", sDate2 = "", sDate3 = "",sExtDate1 = "",sExtDate2 = "", sExtDate3 = "", sExtDate4 = "";
                        DateTime CDate1 = DateTime.Now, CDate2 = DateTime.Now, CDate3 = DateTime.Now;
                        DateTime extDate1 = DateTime.Now, extDate2 = DateTime.Now, extDate3 = DateTime.Now, extDate4 = DateTime.Now;
                        //
                        if (dtStartDate.Text != "")
                        {
                        ...
                        //
                        if (dtExtendedEndDate1.Text != "")
                        {
                            sExtDate1 = dtExtendedEndDate1.Text;
                            //sDate3 = sDate3.Split('/')[1] + "/" + sDate3.Split('/')[0] + "/" + sDate3.Split('/')[2];
                            sExtDate1 = getdt.ConvertDateFormat(sExtDate1);
                           // extDate1 = Convert.ToDateTime(sExtDate1);
                        }
                        //
                        if (dtExtendedEndDate2.Text != "")
                        {
                            sExtDate2 = dtExtendedEndDate2.Text;
                            //sDate3 = sDate3.Split('/')[1] + "/" + sDate3.Split('/')[0] + "/" + sDate3.Split('/')[2];
                            sExtDate2 = getdt.ConvertDateFormat(sExtDate2);
                           // extDate2 = Convert.ToDateTime(sExtDate2);
                        }
                        //
                        if (dtExtendedEndDate3.Text != "")
                        {
                            sExtDate3 = dtExtendedEndDate3.Text;
                            //sDate3 = sDate3.Split('/')[1] + "/" + sDate3.Split('/')[0] + "/" + sDate3.Split('/')[2];
                            sExtDate3 = getdt.ConvertDateFormat(sExtDate3);
                           // extDate3 = Convert.ToDateTime(sExtDate3);
                        }
                        //
                        if (dtExtendedEndDate4.Text != "")
                        {
                            sExtDate4 = dtExtendedEndDate4.Text;
                            //sDate3 = sDate3.Split('/')[1] + "/" + sDate3.Split('/')[0] + "/" + sDate3.Split('/')[2];
                            sExtDate4 = getdt.ConvertDateFormat(sExtDate4);
                           // extDate4 = Convert.ToDateTime(sExtDate4);
                        }
                        ...
                        int Cnt = getdt.InsertorUpdateWorkPackage(WorkPackageUID, new Guid(DDLProject.SelectedValue), new Guid(DDLWorkpackageOption.SelectedValue), new Guid(DDLContractor.SelectedValue), DDLWorkPackage.SelectedItem.Text, DDLLocation.SelectedItem.Text, DDLClient.SelectedItem.Text, CDate1, CDate2, CDate3,
                            ddlstatus.SelectedValue, Convert.ToDouble(txtbudget.Value), Convert.ToDouble(txtActualExpenditure.Value), (DDLCurrency.SelectedItem.Text == "₹ (RUPEE)") ? "&#x20B9;" : (DDLCurrency.SelectedItem.Text == "$ (USD)") ? "&#36;" : "&#165;", Currecncy_CultureInfo, sExtDate1, sExtDate2, sExtDate3, sExtDate4);
                        if (Cnt > 0)
                        {
